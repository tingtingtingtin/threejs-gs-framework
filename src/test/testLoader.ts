...
async function runSplatLoaderTest() {
  ...
  try {
    ...
    if (positions.length === expectedPosLength) {
      ...
      console.error(
        `❌ Positions length Mismatch! Expected ${expectedPosLength}, got ${positions.length}`
      );
    }
    ...
    console.log(
      `Positions[${firstSplatIndex}]: ${positions[0].toFixed(3)}, ${positions[1].toFixed(3)}, ${positions[2].toFixed(3)}`
    );
    // Scales (usually small positive floats)
    console.log(
      `Scales[${firstSplatIndex}]: ${scales[0].toFixed(3)}, ${scales[1].toFixed(3)}, ${scales[2].toFixed(3)}`
    );
    // Rotations (quaternion components, should be between -1.0 and 1.0)
    console.log(
      `Rotations[${firstSplatIndex}]: ${rotations[0].toFixed(3)}, ${rotations[1].toFixed(3)}, ${rotations[2].toFixed(3)}, ${rotations[3].toFixed(3)}`
    );
    console.log(
      `ColorsFloat[${firstSplatIndex}]: R=${colorsFloat[0].toFixed(3)}, G=${colorsFloat[1].toFixed(3)}, B=${colorsFloat[2].toFixed(3)}`
    );
